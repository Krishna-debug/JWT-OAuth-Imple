package com.krishna.JWT;

import com.krishna.JWT.authenticationProviders.JWTAuthenticationProvider;
import com.krishna.JWT.filters.JWTAuthenticationFilter;
import com.krishna.JWT.filters.JWTRefreshFilter;
import com.krishna.JWT.filters.JwtValidationFilter;
import com.krishna.JWT.util.JWTUtil;
...
public class SecurityConfigForJWT {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http, AuthenticationManager authenticationManager,
		...
		http.authorizeHttpRequests(auth -> auth.requestMatchers("/api/user-register").permitAll()
				.requestMatchers("/api/users").hasRole("USER").anyRequest().authenticated())
				.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				.csrf(csrf -> csrf.disable()).addFilterBefore(jwtAuthFilter, UsernamePasswordAuthenticationFilter.class) // generate
																															// token
																															// filter
